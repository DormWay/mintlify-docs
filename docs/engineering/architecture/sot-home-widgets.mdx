...
### iOS Today Surface
- Current: Feed-first Today uses `HomeTabView` + `FeedView` with `WidgetStackView` embedded; widget-only mode is deprecated.
- Target: Maintain feed-first Today while expanding canonical widgets (UpNext + collapsible stack) and folding in HeadsUp when available.
...
## iOS Unified Widget System (January 2026)

The iOS app implements a unified widget system matching the iPad Dashboard's glassmorphic design language.
Today is feed-first on iOS; the widget stack is embedded within the Feed view rather than presented as a standalone surface.
...
### iOS (DormWay) - Unified Widget System
...
## Recent Changes
- 2026-01-05: Documented feed-first Today on iOS; deprecated widget-only mode while keeping UpNext + collapsible widget stack embedded in Feed.
