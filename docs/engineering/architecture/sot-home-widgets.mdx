...
- Cue widget pipeline (cached feed cards + dismissals) used on Home V2.
- **iOS unified widget system** - CollapsibleWidgetContainer, UpNextCard, WidgetStackView for iPhone/iPad.

## Non-goals
- iOS WidgetKit home screen widgets (separate package: DWWidgetKit).
...
- Feed scoring internals beyond the Cue widget contract.
...
- Plan: `obsidian-vault/DormWay/Plans/Workplan/Schedule Widget V2 - Control & Suggestions Plan.md` (schedule control + suggestion UX plan).

### iOS Today Surface
- Current: Today uses `TodayDashboardView` as the single surface (dashboard stack + Cue strip). `FeedView` and `WidgetStackView` are retired.
- Target: Keep the dashboard-first Today and add Work Session entry points from Next Action and Schedule.
...
- Today widget prefers local time when user timezone preference is `auto` (browser/IP timezone fallback), while explicit timezone preferences remain honored.
- Today widget resolves `nextEvent` from a 7-day schedule range (`/v2/students/me/calendar/events`) and falls back to the schedule timeline or `/v2/widgets/today` if the range is unavailable.
- Today widget weather prefers local conditions via `/geo/ip-location` + `/v2/cities/nearest?include=weather,forecast`, then falls back to the weather widget payload or `/v2/widgets/today`.
- Manual weather refresh calls `POST /v2/cities/:id/weather/refresh`, which triggers the `processSingleCity` Temporal workflow when the city weather is stale.
...
- Schedule timeline may emit optional `walkingTimeMinutes` on timed events when two consecutive events both include valid `locationRef.lat/lng` and the later event starts after the earlier event ends.
- Schedule timeline does not emit synthetic walking-time fallbacks when coordinates are missing or transitions overlap.
...
- Web quick actions now use a shared descriptor + execution layer across CommandPalette, Today Inspector, Ace quick actions, Academic Pulse, and Course Drawer.
- `/v2/widgets/batch` now includes contextual `actions[]` metadata for relevant Today, Tasks, and Courses payloads.
- Shared resolver now includes deterministic context-aware quick-action ordering hooks; richer ranking signals remain Phase 4 follow-up.
...
- Deprecated Home widgets may still appear from legacy layouts, but they render an archived placeholder and are removed on save.
...
- Cue uses cached feed batches + `feed_dismissals` filtering; `includeEmpty: true` yields empty content with a version tied to cache + dismissals.
...
## iOS Unified Widget System (January 2026, legacy)

The iOS app implements a unified widget system matching the iPad Dashboard's glassmorphic design language.
Note: the widget stack is not wired into the current Today surface and is treated as legacy UI.

### Widget Architecture

```mermaid
flowchart TD
    subgraph "Today Tab (Legacy)"
        HTV[HomeTabView]
        WSV[WidgetStackView]
        UNC[UpNextCard]
    ...
    subgraph "Collapsible Widgets (Legacy)"
        CWC[CollapsibleWidgetContainer]
        SCW[ScheduleCollapsibleWidget]
        TCW[TasksCollapsibleWidget]
        TRW[TermCollapsibleWidget]
        CCW[CampusCollapsibleWidget]
...
| Component | Purpose |
|-----------|---------|
| `CollapsibleWidgetContainer` | Reusable glassmorphic container with collapse/expand behavior |
| `WidgetStackView` | Container for all 4 widgets + data loading orchestration |
| `UpNextCard` | Current/next event display with quick actions (Start Notes, Record) |
| `HomeWidgetState` | Widget expansion state management with auto-expand logic |

### Design Features
- **Glassmorphic styling**: `.ultraThinMaterial` background with gradient stroke borders
- **Collapsible behavior**: Header tap expands/collapses with spring animation
- **Glance content**: Collapsed state shows quick summary (next event, overdue count)
- **Auto-expand**: Tasks widget auto-expands when overdue items exist
- **iPad parity**: Matches iPad Dashboard GlassSection aesthetic

### Widget Data Flow (Legacy)

```mermaid
sequenceDiagram
    participant WSV as WidgetStackView
    participant HWS as HomeWidgetDataService
    participant API as API Router

    WSV->>+HWS: fetchAllWidgetData()
    HWS->>+API: GET /v2/widgets/home-bundle
    API-->>-HWS: HomeWidgetBundle
    HWS-->>-WSV: Mapped widget data
    WSV->>WSV: Update scheduleData, tasksData, termData, campusData
    WSV->>WSV: autoExpandIfNeeded()
```

## Data Models / IDs / Terminology
- WidgetEnvelope: `{ id, type, gridHints, content, generatedAt }` returned by `/v2/widgets/batch`.
- WidgetTypeId: frontend union in `widgetV2Types.ts` (Home V2 uses `today`, `schedule`, `tasks`, `courses`, `headsup` (Cue) by default).
...
- Schedule timeline event: may include `locationRef` and optional `walkingTimeMinutes` estimate on eligible transitions.
...
- `services/api-router/src/services/headsup/headsup-types.ts` (Cue types + limits; file name uses headsup)
...
### iOS (DormWay) - Unified Widget System (Legacy)
- `.repos/ios-clean/App/Views/Home/Widgets/CollapsibleWidgetContainer.swift` (glassmorphic container with collapse/expand)
- `.repos/ios-clean/App/Views/Home/Widgets/WidgetStackView.swift` (container for 4 collapsible widgets + data loading)
- `.repos/ios-clean/App/Views/Home/Widgets/UpNextCard.swift` (current/next event with quick actions)
- `.repos/ios-clean/App/Views/Home/Widgets/ScheduleCollapsibleWidget.swift` (schedule widget)
- `.repos/ios-clean/App/Views/Home/Widgets/TasksCollapsibleWidget.swift` (tasks widget)
- `.repos/ios-clean/App/Views/Home/Widgets/TermCollapsibleWidget.swift` (term/courses widget)
- `.repos/ios-clean/App/Views/Home/Widgets/CampusCollapsibleWidget.swift` (campus info widget)
- `.repos/ios-clean/App/Views/Home/IPadHomeView.swift` (iPad home uses WidgetStackView)
- `.repos/ios-clean/Packages/DWDesign/Sources/DWDesign/Components/Cards/UniversalCourseCard.swift` (glassmorphic course cards)

### Documentation
- `obsidian-vault/DormWay/Engineering/Architecture/Home V2 Batch-First Widgets (Long-Term Spec).md`
- `obsidian-vault/DormWay/Engineering/Architecture/Onboarding Discovery & First-Run Strategy (Current).md`
...
- `obsidian-vault/DormWay/Plans/Workplan/Quick-Actions-End-to-End-Implementation-Plan.md`
...
- Changing Cue rules (system identifier: headsup): update `headsup-builder.ts`, `headsup-types.ts`, and `useHeadsUpDismiss.ts` integration.

## Recent Changes
- 2026-02-10: Documented DORM-749 schedule timeline walking-time contract (`walkingTimeMinutes`) and coordinate/transition gating rules.
- 2026-02-06: Added shared quick-action execution notes and widget `actions[]` metadata contract updates for Today/Tasks/Courses.
- 2026-02-06: Added cross-surface quick-actions implementation audit and linked the end-to-end execution plan for LockedIn web + api-router.
- 2026-02-01: Added onboarding discovery and first-run strategy for the Today surface, including hint layering, ghost data, and SetupWidget extensions.
- 2026-01-22: Today widget weather compacted (smaller panel, night-aware icon, refresh on open) and header hierarchy simplified (no “Today” label).
- 2026-01-20: Refined Today + Courses widget hierarchy (smaller “Up next” typography, reduced label emphasis) and standardized course code display to avoid badge styling.
- 2026-01-20: Today widget surfaces a context-aware “Take Notes” CTA when the user is in class, deep-linking into Notes Hub with course context.
- 2026-01-16: Added Schedule Widget V2 control + suggestion plan.
- 2026-01-13: Removed `FeedView`/`WidgetStackView` from the Today surface; Cue strip + dashboard stack are now the only Today UI. Added Work Session entry points.
- 2026-01-04: **iOS Unified Widget System** - CollapsibleWidgetContainer with glassmorphic styling, UpNextCard for current/next event with quick actions, WidgetStackView as container for 4 collapsible widgets (Schedule, Tasks, Term, Campus).
...
- 2025-12-24: Expanded student life surfaces spec with OS hierarchy, beta gating, and Cue governance (HeadsUp in spec name).
