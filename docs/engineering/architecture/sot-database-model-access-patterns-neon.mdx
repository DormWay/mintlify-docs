...
- Infra provisioning details for Neon services.

> **Schema Documentation**: For full schema and column-level documentation, see:
> - [Database Table Index](/docs/engineering/technical/database/database-table-index) - Complete table inventory (118+ tables)
> - [Context System Database Schema](/docs/engineering/technical/database/context-system-database-schema) - Core schema details
...
- For compatibility with RLS policies that use `auth.uid()` / `auth.role()`, session queries also set `request.jwt.claim.sub` and `request.jwt.claim.role`.
...
sequenceDiagram
    ...
    Client->>DB: SET LOCAL request.jwt.claim.sub
    Client->>DB: SET LOCAL request.jwt.claim.role
    Client->>DB: Execute query
    DB-->>Router: Result
```

### API Router Direct DB Usage (Inventory)

- **Current state**: API Router still has significant direct DB access in routes and services (not fully dormway-core only).
- **Inventory**: [API Router DB Access Inventory](/docs/engineering/technical/api/api-router-db-access-inventory) (file-level counts and hotspots).
- **Target**: Routes should delegate to dormway-core entities/services; direct DB usage should be isolated to core adapters or dedicated services.
...
- Adding/maintaining policies that rely on `auth.uid()` / `auth.role()`: confirm `request.jwt.claim.sub` and `request.jwt.claim.role` are set in `aurora-client.ts`.
...
1. **Find tables**: [Database Table Index](/docs/engineering/technical/database/database-table-index)
2. **Understand schema**: [Context System Database Schema](/docs/engineering/technical/database/context-system-database-schema)
3. **Debug queries**: Use `DATABASE_URL_ADMIN` (bypasses RLS)

### Critical Rules

| Rule | Why |
|------|-----|
| Use `accounts` for user data | `onboarding` is transient state |
| Join `contexts` for campus | `accounts.campus_id` doesn't exist |
| Include `fetched_at` in `service_data` queries | Table is partitioned by month |
| Use `DATABASE_URL_ADMIN` for debugging | `DATABASE_URL` has RLS that may hide data |
...
## Recent Changes
- 2026-01-26: Added API Router direct DB usage inventory and clarified migration target.
- 2026-01-07: Added schema documentation links, AI agent quick reference section.
