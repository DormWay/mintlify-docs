...
**Risk Level**: 🟢 **Low** - All changes are additive or consolidating; no breaking changes to existing functionality.
...
│ States:  pending → scheduled → completed        │
│ Stores:  Unscheduled work + lifecycle metadata  │
│                                                 │
│ When pending:    Shows in Task Bank             │
│ When scheduled:  Links via scheduled_block_id   │
│ When completed:  Hidden, tracked for analytics  │
└─────────────────────────────────────────────────┘
                    ↓ FK: scheduled_block_id
┌─────────────────────────────────────────────────┐
│ student_time_blocks table                       │
│ ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ │
│ Purpose: ALL scheduled time on timeline         │
│ Types:                                          │
│  - Classes (source='schedule_import')           │
│  - Events (source='calendar_sync')              │
│  - Scheduled tasks (source='manual_planner')    │
│  - Focus blocks (source='ai_suggestion')        │
└─────────────────────────────────────────────────┘
```
...
> **Alternative 2: Extend student_time_blocks** ❌ REJECTED
>
> **Why Rejected**:
> - ❌ Semantic mismatch: "time block" implies scheduled time
> - ❌ Requires nullable `start_time` and `end_time` (breaks constraints)
> - ❌ Mixes two distinct concepts (events vs tasks)
> - ❌ Harder to query ("show unscheduled time blocks" is confusing)
...
1. **Database Schema** ✅
   ...
   - Foreign key: `scheduled_block_id → student_time_blocks(id)`
...
### ⚠️ What's Incomplete (Phase 1 Gaps)
...
{onEventDelete && (event.type === 'task' || event.type === 'planner_block') && (
  <button onClick={() => onEventDelete(event)}>×</button>
)}
```
...
│                                                                  │
│ ┌─────────────────┐    ┌──────────────────┐                    │
│ │ Dashboard Query │    │ Tasks Query      │                    │
│ │ (missing tasks) │    │ (status=pending) │                    │
│ └────────┬────────┘    └────────┬─────────┘                    │
│          │                      │                               │
│          └──────────┬───────────┘                               │
│                     ↓                                            │
│          Client-side merge (TaskBankList.tsx)                   │
│                                                                  │
│ Risk: Data inconsistency, duplicate rendering                   │
└──────────────────────────────────────────────────────────────────┘
...
│          ↓         ↓                                             │
│   ┌──────────┐  ┌────────────────┐                             │
│   │ Tasks API│  │ Planner Blocks │ ← Legacy fallback           │
│   │ (new)    │  │ API (old)      │                             │
│   └──────────┘  └────────────────┘                             │
│                                                                  │
│ Risk: Two systems, inconsistent data model                      │
└──────────────────────────────────────────────────────────────────┘
```
...
1. ✅ Make `tasks` table the single source of truth for unscheduled work
2. ✅ Eliminate client-side task merging (move to backend)
3. ✅ Remove planner blocks fallback (convert ephemeral → persistent first)
4. ✅ Simplify timeline event type logic
5. ✅ Maintain 100% backward compatibility during rollout
...
const allTasks = [
  ...
  ...persistentTasksFormatted, // ✅ ADD THIS
];
```
...
**Impact**:
- ✅ All tasks visible in dashboard composite
- ✅ Frontend can remove separate `useTasks()` call
- ✅ Single query replaces client-side merge
...
if (!isPersistentTask) {
  ...
  console.log('[DragDrop] Converted ephemeral → persistent:', {
    original: task.id,
...
**Testing**:
```bash
# Test ephemeral → persistent conversion
...
7. Hover → Delete button visible
8. Click delete → Task returns to Task Bank
9. Mark complete → Task disappears
...
5. Refresh page → Task still scheduled (persistence verified)
...
|------|----------|------------|
| Dashboard query slows down | 🟡 Medium | Tasks table has indexes; query is simple SELECT |
| Task conversion fails | 🟡 Medium | Wrap in try/catch, fallback to planner blocks |
| Delete confusion | 🟢 Low | Use source field for clear logic |
| Data migration needed | 🟢 Low | No migration - new behavior affects new tasks only |
...
2. **Code Quality**
   - ✅ Client-side merge removed
   - ✅ Dual planner blocks path removed
   - ✅ Delete logic uses source field
...
3. **Functional Correctness**
   ...
   - ✅ Delete returns scheduled tasks to Task Bank

4. **Performance**
   - ✅ Dashboard composite < 200ms (p95)
   - ✅ Frontend queries reduced by 1
   - ✅ No N+1 query issues
...
- Deploy ephemeral → persistent conversion
...
1. ✅ **Aligns with Approved Architecture**: Follows [Task Management System - Persistent Storage Architecture](/docs/engineering/architecture/task-management-system-persistent-storage-architecture) design exactly
2. ✅ **Low Risk**: All changes are additive; easy rollback path
3. ✅ **High Value**: Fixes data consistency issues and simplifies codebase
4. ✅ **Quick Win**: ~1 day of engineering time for significant quality improvement
5. ✅ **Foundation for Future**: Unblocks Phase 2 and Phase 3 work

**The alternative (keeping current state)** means:
- ❌ Incomplete implementation remains indefinitely
- ❌ Technical debt accumulates
- ❌ Developer confusion continues
- ❌ Future phases blocked by incomplete foundation
...
5. **Rollout**: Comfortable with staging → production phased rollout?
