...
```json
{
  "appearance": {
    ...
    "canvasTintPreset": "none|dawn|sky|sage|lavender|sand|rose|slate",
    "canvasTintStrengthLight": 8,
    "canvasTintStrengthDark": 4,
    "reduceMotion": false,
    "highContrast": false,
...
- `canvasTint*` customizes the background canvas only. Surfaces, semantic colors, and urgency tokens remain canonical.

### Canvas Tint Contract (V1)
- Entry point: `Settings > Appearance` only (no global/nav quick toggle).
- Controls:
  - Preset chips (`none|dawn|sky|sage|lavender|sand|rose|slate`)
  - Strength slider with mode-specific ranges:
    - light: `0..18`
    - dark: `0..14`
- Glass guardrail:
  - Effective strength auto-capped in glass mode to preserve readability:
    - light cap `11`
    - dark cap `8`
- Runtime behavior:
  - Applies to ambient canvas background only.
  - Does not recolor cards, borders, typography, or semantic status tokens.
- Rollout:
  - Gated by `NEXT_PUBLIC_FEATURE_CANVAS_TINT_V1` (`FeatureFlags.CANVAS_TINT_V1`).

---

## Preference Normalization & Compatibility

### Why This Matters
Smart Schedule and Schedule Rules depend on **quiet hours**, **working hours**, and **display timezone**. Mixed naming conventions (camelCase in web vs snake_case in core defaults) can cause settings to be ignored.

### Canonical Contract (Schedule-Critical)
Use one canonical shape for schedule-related preferences and accept legacy aliases for reads until cleanup is complete:
...
### Storage Guidance
- Server must normalize into the canonical shape on write and return canonical shape on read.
- UI should read/write only canonical keys once normalization lands.

---

## Smart Schedule Integration (Settings → Scheduling Behavior)

Schedule behavior must respect these preferences:
- `calendar.displayTimezone`: parse intent + render blocks in user’s display timezone.
- `calendar.workHours`: default time window for auto placement.
- `notifications.quietHours`: do not auto-place inside quiet hours unless explicitly requested.
- `calendar.weekStart` + `calendar.defaultView`: set initial Schedule view state.
- `calendar.timeFormat`: render schedule time labels; does not affect placement.

### UX Implications
- When a user changes working hours or quiet hours, future rule materializations use the new windows.
- UI should show a brief toast when schedule preferences change and optionally offer “rebuild upcoming suggestions.”
