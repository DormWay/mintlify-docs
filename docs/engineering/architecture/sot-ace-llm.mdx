...
- Context aggregation is controlled by `USE_RICH_CONTEXT` feature flag (defaults to `true`). When disabled, falls back to RAG-only mode.
 - AI policy gate is enforced before any Ace chat LLM call; when `ai.enabled` is `false`, the request must be blocked (403) or short-circuited with a non-AI fallback. (`services/api-router/src/services/ai-policy.ts`, `services/api-router/src/routes/ace-llm-chat-routes.ts`)
...
- `obsidian-vault/DormWay/Engineering/Architecture/Help Center & Ace Knowledge Coverage (Current).md` - Help Center coverage + Ace help ingestion plan
...
- **2026-02-01**: Documented Help Center coverage gap and recommended Ragie ingestion path for product how-to support.
...
- **Help Center coverage**: Product how-to docs are not indexed in Ragie, so Ace can only answer from structured data + user/course documents.
