...
- Supported signals: `daily_plan`, `context_update`, `generate_plan`, `preferences_update`, `trigger_calendar_sync`, `trigger_canvas_sync`, `send_evening_briefing`, `generate_feed_cards`.
...
- Start on onboarding: `studentProcessor.workflow` calls `startStudentWatcher`, which resolves timezone and starts `studentWatcherWorkflow`.
- Start on Clerk signup: `clerk-routes` attempts `startWorkflow('studentWatcherWorkflow')` with default timezone; failures are logged but do not block signup.
- Admin-driven signals: `dayplan-admin-actions` uses `generate_plan` for regeneration/email; `preferences-routes` signals `preferences_update`.
- Runtime loop: handles `context_update` signals, regenerates plans on calendar/syllabus/enrollment changes, runs schedule/Canvas auto-syncs, and saves state to `student_watcher_state`.
