...
- Canvas sync upserts assignment-backed task rows with `source='assignment'`, `source_type='canvas'`, and `source_id=canvas_assignments.id`; submissions mark tasks completed. (`services/engine/src/activities/canvas.activities.ts`)
...
- Assignment completion IDs are canonical: Canvas uses `canvas-<canvas_assignment_id>` (numeric LMS ID), syllabus uses `syllabus-<courseContextId>-<index>`, and generic assignments use UUIDs from `tasks.assignment_id`.
- Manual completion updates assignment-backed `tasks` rows (Canvas by `metadata.canvasAssignmentId`, generic by `assignment_id`) so Tasks V2 remains consistent with `assignment_completion`.
...
- `/v2/students/me/task-bank` returns unified unscheduled work (DayPlan suggestions, Canvas, quick captures, persistent tasks) with server-side canonicalization (`canonical_id`, `canonical_key`, `sources`, `has_time`, `due_date`). (`services/api-router/src/routes/v2/students.routes.ts`, `services/api-router/src/utils/task-bank-canonicalization.ts`)
- `/api/mobile/planner/task-bank` shares the same canonicalization helper for cross-surface parity. (`services/api-router/src/routes/mobile-routes.ts`, `services/api-router/src/utils/task-bank-canonicalization.ts`)
...
- Courses term insights pull assignments via `/v2/students/me/current-term/assignments` and map them into `summary.course_assignments` in `useSemesterAggregate`. (`services/api-router/src/routes/v2/students.routes.ts`, `services/dormway-lockedin/src/hooks/useSemesterAggregate.ts`)
- V2 term insights payloads (summary/workload/insights/warnings/conflicts/analysis + assignments) are validated with Zod before returning. (`services/api-router/src/routes/v2/students.routes.ts`)
...
- Smart Schedule intent scheduling writes manual time blocks directly and does not create tasks unless the source is Task Bank. (`services/api-router/src/routes/v2/students.routes.ts`, `services/dormway-lockedin/src/app/dashboard/schedule/page.tsx`)
...
- Term data surface map (Courses/Today/Tasks): [Term Data Surface Audit](/docs/engineering/technical/lockedin/term-data-surface-audit)
...
- Task Bank canonicalization (Phase 1 implemented; Phase 2 persistence in progress): shared helper across `/v2/students/me/task-bank` + `/api/mobile/planner/task-bank`, canonical_key uses display timezone + date-only detection, normalize source types, dedupe by source_id then canonical_key, canonical_id = tasks.id or hashed key. Spec: [Task Bank Canonicalization Spec (Draft)](/docs/engineering/architecture/task-bank-canonicalization-spec-draft)
...
sequenceDiagram
    participant Client
    participant API as /api/mobile/planner/task-bank or /v2/students/me/task-bank
    participant DB as PostgreSQL
    ...
    API->>API: Canonicalize via source_id + canonical_key (canonicalizeTaskBank)
...
- Task canonical fields: `canonical_key`, `source_type`, `source_id`, `source_priority`. (`infrastructure/database/migrations/20260116_add_task_canonical_fields.sql`)
...
- Task Bank (V2): same merge logic returned at `/v2/students/me/task-bank` for web surfaces. (`services/api-router/src/routes/v2/students.routes.ts`)
...
- `services/dormway-lockedin/src/components/courses/insights/AssignmentTimelineCard.tsx`
- `services/dormway-lockedin/src/components/courses/TermInsightsContent.tsx`
...
## Recent Changes
- 2026-01-28: Added V2 term assignments endpoint and migrated Courses term insights to V2 (`useSemesterAggregate`).
- 2026-01-28: Added Zod validation for V2 term insights responses (including plan/trends).
- 2026-01-04: Documented Smart Schedule intent scheduling as manual time blocks distinct from task lifecycle.
- 2026-01-04: Added `/v2/students/me/task-bank` as the web-facing Task Bank endpoint.
