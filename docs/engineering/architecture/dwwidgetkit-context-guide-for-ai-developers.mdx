...
**DormWay is an AI-powered College Command Center** that helps students manage every aspect of their campus life—automatically populating schedules, tracking assignments, recommending events, and providing contextual assistance without overwhelming students with complexity.
...
- Need information in under 5 seconds
...
│                    Client Layer                         │
├──────────────┬──────────────┬──────────────┬───────────┤
│  iOS App     │  Widgets     │  LockedIn    │  Admin    │
│  (SwiftUI)   │  (WidgetKit) │  (Next.js)   │  (React)  │
└──────────────┴──────────────┴──────────────┴───────────┘
                         ▼
┌─────────────────────────────────────────────────────────┐
│           Zuplo API Gateway (Port 9000)                 │
│  Auth0/Clerk JWT validation, Rate limiting, Routing     │
└─────────────────────────────────────────────────────────┘
                         ▼
┌─────────────────────────────────────────────────────────┐
│                  Backend Services                       │
├──────────────┬──────────────┬──────────────┬───────────┤
│ API Router   │  Engine      │  DW Crews    │ Ably      │
│ (Express)    │  (Temporal)  │  (FastAPI)   │ (Real-time)│
│ Port 3001    │  Port 3030   │  Port 8000   │ Port 3032 │
└──────────────┴──────────────┴──────────────┴───────────┘
                         ▼
┌─────────────────────────────────────────────────────────┐
│  PostgreSQL + PostGIS (5432) | Redis (6379)             │
│  User data, Contexts, Spatial | Cache, Sessions         │
└─────────────────────────────────────────────────────────┘
                         ▼
┌─────────────────────────────────────────────────────────┐
│                 External Services                       │
│  Canvas LMS | Google Cal | OpenAI | Customer.io         │
│  Auth0/Clerk | Ably | APNS | AWS (S3, ECS)              │
└─────────────────────────────────────────────────────────┘
```
...
❌ Study groups, grade tracking, dining integration, transportation, social networking, course selection, professor ratings, textbook marketplace, note-taking, flashcards
...
- Glance at home screen → see next class
- Glance at home screen → see AI recommendation
- Glance at home screen → see assignments due today
...
1. **Glanceable** - Information in under 5 seconds, no scrolling
...
- ✅ 2 widgets implemented (Ace, Next Action)
- ✅ DWDesign system established
- ✅ Backend composite API operational
- ⚠️ Real-time updates (not yet integrated)
- ⚠️ Additional widget types (not yet built)
...
## 🎯 How Widgets Fit Into User Journeys
...
5. **Tap widget** → Opens app to relevant section
...
- ✅ Yes → Prioritize
- ❌ No → Deprioritize (focus on core journeys)

### 2. Is this glanceable?
- ✅ Information visible in under 5 seconds → Good widget
- ❌ Requires scrolling or interaction → Reconsider

### 3. Does this use real backend data?
- ✅ Yes, from composite API → Correct
- ❌ Using mock data → Fix before proceeding

### 4. Is this context-aware?
- ✅ Shows relevant info for time/location → Good
- ❌ Generic, not personalized → Add context filtering

### 5. Does this follow DWDesign?
- ✅ All styling from DWDesign tokens → Correct
- ❌ Hardcoded fonts/colors/spacing → Fix immediately

### 6. Is this privacy-respecting?
- ✅ No sensitive info on lock screen → Good
- ❌ Shows grades/private data → Add privacy filter
...
1. **Glanceable** - Info in under 5 seconds
...
**Welcome to DormWay! You're building something that will help thousands of students succeed. Let's make it amazing. 🚀**
