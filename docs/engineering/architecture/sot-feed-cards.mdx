...
**2026-02-22 contract addendum (`day_conditions`):**
- `day_conditions` may include structured `revealContent` for body parsing (`Conditions` / `Planning` / `Be Prepared`) even when `hasReveal` is `false`.
- Treat `hasReveal` as the source of truth for expand/collapse behavior; do not infer expandability from `revealContent` presence alone.
...
- [Dashboard Intelligence](/docs/engineering/dashboard-intelligence/readme) - Dashboard BFF patterns
