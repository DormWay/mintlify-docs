...
flowchart LR
  ...
  subgraph Edge["Edge / Gateway"]
    Zuplo["Zuplo Gateway dormway-api-2"]
  end

  subgraph Backend["Backend dormway-platform"]
    APIRouter[api-router]
    Engine["engine Temporal worker"]
  end

  subgraph Data["Data + External Systems"]
    PG[("Postgres Neon/Aurora")]
...
sequenceDiagram
  ...
  Zuplo->>Zuplo: Verify token ClerkJwtInboundPolicy / Auth0JwtInboundPolicy legacy
  Zuplo->>Zuplo: Add headers x-user-id/x-user-email/x-user-role
...
sequenceDiagram
  ...
  API->>Clerk: Update public metadata backend_user_id + role
  API->>Temporal: Start workflow studentWatcherWorkflow workflowId=student-watcher-backend_user_id
  Temporal-->>Engine: Dispatch to task queue student-worker
  Engine-->>Temporal: Workflow running
  API-->>Clerk: 200 OK ack webhook
  Note over Zuplo: On future requests Clerk JWT includes backend_user_id via template Zuplo forwards x-user-id=backend_user_id
```
...
sequenceDiagram
  ...
  participant LockedIn as dormway-lockedin api-proxy
  participant Zuplo as Zuplo Gateway
  participant API as api-router

  Browser->>LockedIn: fetch api-proxy-path
  LockedIn->>LockedIn: getProxyAuthHeaders from Clerk session cookie
...
sequenceDiagram
  ...
  iOS->>iOS: Optionally sign request X-App-* headers AppHMACSigner
  iOS->>Zuplo: Request
  Zuplo->>API: Forward + x-user-* + gateway-api-key
...
flowchart LR
  ...
  B --> C{campus?}
  C -- yes --> D[students + active students + enrollment + Canvas flags]
  C -- no --> E[city campus growth signals]
  D --> F[base + student_score + activity_score + time_score + Canvas bonus]
  E --> F
...
stateDiagram-v2
  ...
  TickLoop --> MaybeGeneratePlan : if 5 AM user time OR forced
  MaybeGeneratePlan --> GeneratingPlan : generateMorningPlan
  GeneratingPlan --> MaybeNotify : push or email based on prefs
  MaybeNotify --> PersistState : updateNotificationDates
  PersistState --> TickLoop

  TickLoop --> MaybeSyncCalendar : if stale 6h
  MaybeSyncCalendar --> SyncingCalendar : scheduleProcessor
  SyncingCalendar --> PersistState

  TickLoop --> MaybeSyncCanvas : if stale 6h
  MaybeSyncCanvas --> SyncingCanvas : canvas sync workflow
...
flowchart LR
  subgraph Starters["Who starts StudentWatcher?"]
    ClerkWebhook["Clerk webhook user.created"]
  ...
  Engine --> Watcher["studentWatcherWorkflow workflowId=student-watcher-userId"]
```
...
sequenceDiagram
  ...
  Ragie-->>API: POST webhooks/s3/ragie file.indexed / file.updated / ...
  API-->>Ragie: 200 OK ack early
  API->>Temporal: signalWorkflow workflowId=student-watcher-userId signal=context_update
  Temporal-->>Watcher: Deliver signal handler
  Watcher->>Watcher: context_update handler may trigger plan regen / context inference
```
...
```mermaid
flowchart TD
  A["Tick every 5 min"] --> B{"5:00 AM user time and new day?"}
...
flowchart TD
  ...
  B -- image --> C["Vision/OCR path imageData"]
  B -- pdf --> D["OpenAI Files upload fileId"]
  B -- ics --> E["ICS parse path parseICSWithGPT5"]
  B -- text --> F["Portkey prompt schedule-ocr-parsing"]
...
sequenceDiagram
  ...
  DB-->>Engine: none
  Engine->>Engine: Use partition user_email_hash
  Engine->>Ragie: index document partition=user_email_hash
  ...
  API-->>API: Log Ragie migration needed email-hash to user_uuid
```
