...
Prompts are synced from Git to Portkey via GitHub Actions (`.github/workflows/sync-prompts.yml`):
- On push to main: syncs and publishes changed prompts
- On PR: dry-run preview of what would be synced
- Script: `scripts/sync-prompts-to-portkey.js`

The sync handles provider mapping (virtual keys), `max_tokens`, `temperature`, and `search_recency_filter` (Perplexity-specific). See Deep Dive SoT on prompts for the full prompt audit.
