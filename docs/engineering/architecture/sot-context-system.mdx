...
description: "sequenceDiagram participant Router as API Router participant Service as ContextGraphService participant Cache as Redis participant DB as PostgreSQL"
---
...
- Course lifecycle read semantics are centralized in `resolveCourseLifecycleState` (`services/shared/dormway-core/src/entities/course-lifecycle.ts`).
- `resolveCourseLifecycleState` maps `completed`, `dropped`, and `has_taken` dependency states (plus explicit archived flags/status) into canonical lifecycle outputs where `isArchived=true`.
- Term-facing enrollment reads (`TermFactory.getForStudent` / `getAllForStudent`) include `enrolled_in`, `completed`, `dropped`, and `has_taken`, then rank one edge per course with priority `enrolled_in > completed > dropped > has_taken`.

## Key Flows (High-Level)

### API Router Context Access

```mermaid
sequenceDiagram
    participant Router as API Router
    participant Service as ContextGraphService
    participant Cache as Redis
    participant DB as PostgreSQL

    Router->>Service: getContextGraphService(session)
    Service->>Service: Create session-bound clone
    Service->>Cache: Check cache
    Service->>DB: SET LOCAL RLS vars
    Service->>DB: Execute traversal
    DB-->>Router: Context data
```
...
sequenceDiagram
    ...
    participant Service as getAdminContextGraphService
    participant Pool as Admin Pool
    participant DB as PostgreSQL

    Admin->>Service: Get admin service
    Service->>Pool: Get connection (DATABASE_URL_ADMIN)
    Note over DB: RLS bypassed
    Pool->>DB: Execute context query
    DB-->>Admin: Context data
```

### Onboarding v2 Context Creation

```mermaid
flowchart TD
    A[Email domain] --> B[Match campus_configs]
    B --> C{Campus found?}
    C -->|Yes| D[Create/update student context]
    D --> E["Set parent_id = campus_id"]
    C -->|No| F[Error or default handling]
```
...
sequenceDiagram
    participant Caller
    participant Fn as getContextHistory
    participant DB as service_data

    Caller->>Fn: getContextHistory(userId)
    Fn->>DB: SELECT WHERE method='context_prediction_response'
    Fn->>DB: ORDER BY fetched_at DESC
    DB-->>Caller: History records
```
...
- Dependency types include enrollment, lifecycle, and containment relationships (`enrolled_in`, `completed`, `dropped`, `has_taken`, `has_enrolled`, `contains`, etc.).
...
## Recent Changes
- 2026-02-10: Documented canonical lifecycle resolver and TermFactory lifecycle dependency/ranking behavior (DORM-959).
