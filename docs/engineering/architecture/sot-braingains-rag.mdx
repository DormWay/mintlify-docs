...
- When `ai.enabled` is `false`, BrainGains chat and Ragie indexing must be skipped or blocked for that user (no AI processing on opt-out data). (`services/api-router/src/services/ai-policy.ts`, `services/api-router/src/routes/braingains-chat-routes.ts`)
...
## Recent Changes
- 2026-01-25: Syllabus processing enriches crew output with term window data so downstream calendar normalization can resolve Week/Finals references.
