...
- Schedule import: `scheduleImport.workflow` parses a schedule, then calls `normalizeAndStoreCalendarData` with campus timezone and `source='schedule_import'` to write context time blocks; reconciliation to student-facing blocks happens via schedule processing.
- Schedule reconciliation: `scheduleProcessor` pulls Google/LMS/mobile/syllabus/enrolled course events, normalizes, consolidates, and stores them, then publishes an Ably dashboard update.
- API reads:
