...
## ⚠️ CRITICAL: DWDesign is the Source of Truth

**BEFORE YOU START**: DWDesign (`Packages/DWDesign/`) is the **single source of truth** for ALL design decisions in this project.

### MANDATORY RULES

- ❌ **NEVER** hardcode font sizes, weights, or styles - Use `DS.Typography.*` tokens
- ❌ **NEVER** hardcode colors - Use `DS.Color.Semantic.*` or `DS.Color.Palette.*` tokens
- ❌ **NEVER** hardcode spacing values - Use `DS.Spacing.*` tokens
- ❌ **NEVER** hardcode corner radius - Use `DS.Radius.*` tokens
- ✅ **ALWAYS** import `DWDesign` in widget view files
- ✅ **ALWAYS** use `.dsTypography()` modifier for text styling
...
├── DormWayWidgets.swift (@main WidgetBundle)
├── Info.plist
...
// ❌ WRONG - Never use system colors directly
Text("Error message")
    .foregroundColor(.red)
```
...
```swift
// ✅ CORRECT - Course color as accent
HStack {
    Circle()
...
// ❌ WRONG - Don't use course color for all text
Text(course.code)
...
- ✅ All colors use `DS.Color.Semantic.*` or `DS.Color.Palette.*`
- ✅ All spacing uses `DS.Spacing.*`
- ✅ All corner radius uses `DS.Radius.*`
- ✅ Course colors used only as accents
- ✅ Loading state implemented and styled
- ✅ Error state implemented with helpful message
- ✅ Empty state implemented if applicable
- ✅ VoiceOver labels added for accessibility
- ✅ Text truncation tested for long content
- ✅ Both small and medium sizes implemented
- ✅ Dark mode appearance verified (semantic colors auto-adapt)
- ✅ Swift 6.0 concurrency patterns followed
- ✅ No `@MainActor` on Timeline providers
...
- [DormWay iOS App - Complete Package & Architecture Guide](/docs/engineering/technical/ios/dormway-ios-app-architecture) - Full app architecture
