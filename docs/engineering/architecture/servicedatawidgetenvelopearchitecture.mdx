...
# Service Data → Widget Envelope Architecture

## Executive Summary

**Foundation Laid**: ✅ Complete scalable architecture for widget envelope generation
...
│  Consumes: DashboardCompositeV1 with typed WidgetEnvelope[] │
└─────────────────────────────────────────────────────────────┘
                              ▲
                              │ HTTP GET /dashboard/v1/composite
                              │ ETag caching, 304 Not Modified
                              │
┌─────────────────────────────────────────────────────────────┐
│                    API RESPONSE LAYER (BFF)                  │
│   dashboard-composite-transformer.ts                         │
│   - Reads from materialized views or service_data           │
│   - Transforms to typed widget envelopes                     │
│   - Applies business logic (urgency, priorities)             │
...
└─────────────────────────────────────────────────────────────┘
                              ...
                              │
┌─────────────────────────────────────────────────────────────┐
│              MATERIALIZED VIEW LAYER (Performance)           │
│   mv_recent_assignments (hot assignments, updated hourly)   │
│   mv_recent_schedule (48-hour window, updated every 15min)  │
│   mv_dayplan_blocks (current day + 7 days, updated 30min)   │
...
└─────────────────────────────────────────────────────────────┘
                              ...
                              │
┌─────────────────────────────────────────────────────────────┐
│               STORAGE LAYER (Partitioned JSONB)              │
│   service_data (partitioned by month)                        │
│   - Flexible JSONB storage for varying data shapes          │
│   - Canvas assignments, calendar events, AI crew outputs    │
│   - Partitioned by fetched_at (monthly)                     │
│   - Pruned automatically (90-day retention)                  │
└─────────────────────────────────────────────────────────────┘
                              ▲
                              │ Writes from
                              │
...
│   Temporal workflows, Canvas API, Google Calendar, AI Crews │
│   - Writes raw data to service_data                         │
│   - JSONB format for flexibility                             │
│   - Timestamped with fetched_at                              │
└─────────────────────────────────────────────────────────────┘
```

---

## Data Flow: Canvas Assignment → Widget Envelope
...
- ✅ Automated pruning (90-day retention)
- ✅ Scheduled MV refresh (15-30 min intervals)
...
- Assignments: 250ms → 12ms (20x)
- Schedule: 180ms → 8ms (22x)
- DayPlan: 120ms → 6ms (20x)
