...
- 401/403/404 and invalid ICS payloads are treated as **non-retryable** to avoid infinite retry loops.
- `canvasICSSync` is started with a `workflowExecutionTimeout` of **10 minutes**.
...
| ICS-only completion telemetry | Treat as unknown completion unless explicit signal exists | Canvas ICS + task status normalization paths |
...
## 7. User-Facing Entry Points (Web)

Canvas connection and sync are exposed in three primary web surfaces:

1. **Settings > Connections > Canvas LMS**
   - Component: `services/dormway-lockedin/src/components/settings/ConnectionsContent.tsx`
   - Integration UI: `services/dormway-lockedin/src/components/canvas/CanvasIntegration.tsx`
2. **Courses > Canvas tab**
   - Wrapper: `services/dormway-lockedin/src/components/courses/CanvasContent.tsx`
3. **Direct Canvas page (legacy route)**
   - Page: `services/dormway-lockedin/src/app/dashboard/canvas/page.tsx`

Help Center reference:
- `obsidian-vault/DormWay/Product/Features/Documentation/Help Center/DormWay LockedIn Help Center Seed.md` (Canvas connection + sync steps)
...
## Recent Changes
- 2026-02-20: Added cross-listed identity + ICS completion-state caveats and linked the canonical implementation plan for tenant-scoped dedupe and capability-aware overdue semantics.
- 2026-02-01: Documented web entry points for Canvas integration and linked Help Center instructions.
- 2026-01-25: Marked 401/403/404 + invalid ICS payloads as non-retryable and added a 10-minute `workflowExecutionTimeout` for `canvasICSSync` to prevent runaway retries.
...
- ICS completion state: many institutions expose due dates but not reliable completion/submission state via ICS feed, so overdue semantics must remain capability-aware.
...
- [DayPlan V2 - State of Things](/docs/engineering/architecture/sot-dayplan-v2)
- [Canvas ICS Sync Implementation](/docs/engineering/technical/api/canvas/canvas-sync-service-implementation)
