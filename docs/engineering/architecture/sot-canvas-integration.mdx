...
- **File**: `services/engine/src/activities/canvas.activities.ts` (lines 210-224)
...
- **Detection logic** (line 1861-1923): Checks for absence of `refresh_token` field

#### OAuth2 Flow
- **File**: `services/api-router/src/routes/canvas-routes.ts` (lines 55-215)
...
- **Token refresh**: Automatic 401 handling with retry (lines 40-82 in canvasClient.ts)
- **Refresh implementation**: Uses `refreshAccessToken()` with exponential backoff
...
- **Connection type detection** (line 1861-1923): ICS marked when `canvas_instance_id` starts with `'ics-'`

### API Endpoints Called

| Endpoint | File Location | Notes |
|----------|---------------|-------|
| `GET /api/v1/courses` | line 287-328 | Active enrollments, includes term/students/enrollments |
| `GET /courses/{id}/assignments` | line 680-704 | Includes score_statistics |
| `GET /courses/{id}/assignments/{id}/submissions/self` | line 710-744 | Submission details |
| `GET /calendar_events` | line 1537-1615 | Class meetings (type='event') |
| `GET /courses/{id}/discussion_topics` | line 1449-1473 | Announcements only |
| `GET /courses/{id}/users` | line 1478-1502 | Classmates |
| `GET /users/self/activity_stream/summary` | line 1971-2077 | Canary check |

### Rate Limiting & Error Handling

**Temporal Retry Policy** (line 42-49 in workflows):
```typescript
{
...
|--------|-------------|--------|---------------|
| 429 | Rate Limited | Exponential backoff with Temporal retry policy | line 223-250 |
| 404 | Not Found | Return null, skip resource gracefully | line 737-743 |
| 401 | Unauthorized | Attempt token refresh (OAuth2 only) | canvasClient.ts:40-82 |
| 403 | Forbidden | Token refresh or fail if PAT | canvasClient.ts:40-82 |
