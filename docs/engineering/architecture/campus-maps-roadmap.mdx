...
| DORM-748 | [Maps P1.1] Add LocationRef to dormway-core | Done |
| DORM-749 | [Maps P1.2] Backend: emit walkingTimes | Done |
...
**Current implementation (DORM-749):**
- Utility shipped in `services/shared/dormway-core/src/utils/walking-time.ts`.
- Haversine distance + buffered ETA (`bufferMultiplier: 1.4` / 40%).
- Exposed on schedule events as optional `walkingTimeMinutes`.
- Emitted only when two consecutive timed events both have valid `locationRef.lat/lng` and the current event starts after the previous event ends.
- No synthetic fallback minutes are emitted when coordinates are missing.
...
export function estimateWalkingTimeMinutes(
  ...
  options?: {
    walkingSpeedMetersPerMinute?: number; // default: 84 (~1.4 m/s)
...
```typescript
// services/api-router/src/services/schedule-timeline.ts
