...
description: "1. ✅ Read the 3 essential guides (90 min) 2. ✅ Explore existing widget code in `.repos/ios-clean/Packages/DWWidgetKit/` 3. ✅ Review DWDesign package in `.rep..."
---
...
1. ✅ Read the 3 essential guides (90 min)
2. ✅ Explore existing widget code in `.repos/ios-clean/Packages/DWWidgetKit/`
3. ✅ Review DWDesign package in `.repos/ios-clean/Packages/DWDesign/`
4. ✅ Test existing widgets on device/simulator

### Phase 2: Backend Integration (Days 2-3)
1. ✅ Update `WidgetDataProvider` to use composite API
2. ✅ Implement device key authentication
3. ✅ Add ETag-based caching
4. ✅ Test with real backend endpoint
5. ✅ Remove any mock data fallbacks

### Phase 3: Enhance Existing Widgets (Days 4-5)
1. ✅ Update Ace widget to use real data
2. ✅ Update Next Action widget to use real data
3. ✅ Polish UI with DWDesign tokens
4. ✅ Add proper loading and error states
5. ✅ Test on physical device

### Phase 4: Add New Widgets (Days 6-8)
1. ✅ Implement Schedule widget
2. ✅ Implement Assignments widget
3. ✅ Implement Academic Momentum widget (optional)
4. ✅ Implement Study Blocks widget (optional)
5. ✅ Add all to WidgetBundle

### Phase 5: Real-Time & Polish (Days 9-10)
1. ✅ Integrate Ably for push updates
...
### 1. DWDesign is LAW
- ❌ **NEVER** hardcode fonts, colors, spacing, or radius
- ✅ **ALWAYS** use `DS.Typography.*`, `DS.Color.Semantic.*`, `DS.Spacing.*`, `DS.Radius.*`
- If you type a number for styling, STOP and find the token!

### 2. Use REAL Backend Data
- ❌ **NO MOCK DATA** in production code
- ✅ Use `GET /dashboard/v1/composite` endpoint
- Return errors if backend unavailable, don't fake data

### 3. Respect Beta Scope
- ✅ Focus on 5 core user journeys
- ❌ Don't build features not in beta scope (study groups, dining, transportation, etc.)
- The line is snapped (locked July 12, 2025)

### 4. Privacy First
- ✅ Consent defaults to FALSE
- ✅ No sensitive info on lock screen (widgets visible to others)
- ✅ Respect user privacy settings

### 5. Context-Aware
- ✅ Use current context from App Groups
- ✅ Pass context parameter to API
- ✅ Show relevant data for time/location/user

### 6. Swift 6.0 Concurrency
- ✅ Use `@MainActor` for UI code
- ✅ Use `async/await` for network calls
- ❌ NO `@MainActor` on TimelineProvider
...
1. **Glanceable** - Information visible in under 5 seconds, no scrolling
...
- **Weather widget**: "Rainy, 50°F - Bring umbrella"
...
- **Academic Momentum widget**: "5-day study streak 🔥 Keep it up!"
...
│   │   │   └── WidgetModels.swift
│   │   ├── Provider/
│   │   │   └── WidgetDataProvider.swift  ← You'll modify this
│   │   ├── Shared/
│   │   │   └── WidgetExtensions.swift
│   │   └── Widgets/
│   │       ├── AceStudyCommandWidget.swift
│   │       ├── NextActionWidget.swift
│   │       └── [New widgets here]
│   └── Package.swift
│
├── Packages/DWDesign/
...
│
├── DormWayWidgets/
│   ├── DormWayWidgets.swift  ← Widget bundle entry point
│   ├── Info.plist
│   └── DormWayWidgets.entitlements
│
└── Core/DependencyInjection/
    └── Container.swift  ← Lines 505-515 (widget DI setup)
```
...
└── services/api-router/src/
    ├── routes/
    │   └── dashboard-routes.ts  ← Composite API endpoint
    ├── middleware/
    │   └── auth.ts  ← Authentication chain (device key)
    └── services/
        ├── ably-service.ts  ← Real-time pub/sub
        └── dashboard-composite-transformer.ts  ← Widget assembly
```
...
├── DWWidgetKit - README for AI Developers.md  ← You are here!
├── DWWidgetKit - Context Guide for AI Developers.md  ← Read first
├── DWWidgetKit - iOS Architecture & Design System Guide.md  ← Read second
└── DWWidgetKit - Backend Integration Guide.md  ← Read third

.workspace/
├── current-state/
│   ├── architecture/
│   │   └── DORMWAY_COMPREHENSIVE_GUIDE.md  ← Platform overview
│   └── beta-specs/
│       ├── WIDGET_DATA_FORMATS.md
│       ├── REALTIME_ABLY_SPECIFICATION.md
│       └── BACKEND_REQUIREMENTS_FOR_BETA.md
└── analysis/
    └── WIDGET_PROMPT_AUDIT.md  ← Why original prompt was wrong
```
...
5. How does the context hierarchy work (City → Campus → Building → Course → Student)?
...
- [ ] I understand the context hierarchy (City → Campus → Building → Course → Student)

**If you've checked all boxes, you're ready to start implementing! 🚀**
...
1. ✅ Widgets fetch real data from `GET /dashboard/v1/composite`
2. ✅ Authentication works via device key (with JWT fallback)
3. ✅ All styling uses DWDesign tokens (no hardcoded values)
4. ✅ Widgets are glanceable (under 5 seconds to understand)
5. ✅ Widgets are context-aware (show relevant data for time/location)
6. ✅ Error states are user-friendly (not technical error messages)
7. ✅ Loading states use cached data (no blank screens)
8. ✅ ETag caching works (304 Not Modified responses)
9. ✅ Real-time updates optional but implemented (Ably)
10. ✅ Tests pass (unit tests for data provider)
11. ✅ Widget views work on both small and medium sizes
12. ✅ Accessibility works (VoiceOver labels, Dynamic Type)
...
1. ✅ **Context** - You understand WHY
2. ✅ **Patterns** - You know HOW (iOS)
3. ✅ **Integration** - You know HOW (Backend)
4. ✅ **Resources** - You know WHERE to look
5. ✅ **Scope** - You know WHAT to build
