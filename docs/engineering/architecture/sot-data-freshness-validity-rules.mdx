...
description: "Status: Implemented in `feat/global-freshness-api` branch (December 2025)."
---
...
- Dashboard request -> Redis cache lookup -> ETag/304 or cached response -> recompute with `getContextTTL` -> cache write and invalidate on change.
- Campus processing -> staleness/missing city datasets -> `processSingleCity` executed synchronously for first-time data or in background for stale/missing data.
- Mobile campus sync status -> `last_modified_at` -> stale flag returned to iOS client.
- Campus enrichment selection -> `getStaleCampusIds` -> list of campus IDs with stale `service_data.updated_at`.
- Weekly cleanup -> time blocks pruned by age/future horizon; duplicate and orphan cleanup reduces invalid records.
- Syllabus processing -> hash + parser version -> cached analysis reused or re-parse when stale.
- **Global Freshness API**: Request -> Redis cache check (key includes options suffix) -> if miss, parallel queries for each requested type -> compute staleness per type -> cache result -> return summary/detailed response.
...
Status: Implemented in `feat/global-freshness-api` branch (December 2025).
...
- 2025-12-25: Added proposed global freshness API contract (not implemented).
- 2025-12-25: Documented setup status endpoint as a related (non-freshness) signal.
- 2025-12-25: **Implemented Global Freshness API** in `feat/global-freshness-api` branch:
