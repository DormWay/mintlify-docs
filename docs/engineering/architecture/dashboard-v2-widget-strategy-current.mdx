...
- Layout API v1 (aliased under v2): `.repos/dormway-platform/services/api-router/src/routes/layout-routes.ts`
...
- Persists layouts via `/api/proxy/api/v2/layouts` (alias of v1; Next proxy → Zuplo → api-router).
...
- Persists widget layouts via `/api/v1/layouts` (aliased under `/api/v2/layouts`) + `LayoutService` (validation + PII stripping + optimistic locking).
...
sequenceDiagram
  ...
  UI->>Proxy: GET /api/proxy/api/v2/layouts?tab=home
  Proxy->>Zuplo: forward (auth injected)
  Zuplo->>API: GET /api/v2/layouts?tab=home
