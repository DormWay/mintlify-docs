...
description: "flowchart TD A[Engine boot] --> B{SYNC_SCHEDULES_ENABLED?} B -->|Yes| C[ensureSyncSchedules] B -->|Yes| D[ensureCleanupSchedules] B -->|Yes| E[ensurePluginRu..."
---
...
- Calendar-triggered sync workflow IDs are bucketed + sharded to dedupe bursts (`schedule-manual-s{shard}-{userId}-{bucket}`).
- Account-level calendar sync execution uses dedupe windows (`calendar-sync:account:{studentId}:{accountId}:{bucket}`) to suppress duplicate fan-out.
...
### Engine Boot Schedule Provisioning

```mermaid
flowchart TD
    A[Engine boot] --> B{SYNC_SCHEDULES_ENABLED?}
    B -->|Yes| C[ensureSyncSchedules]
    B -->|Yes| D[ensureCleanupSchedules]
    B -->|Yes| E[ensurePluginRunnerSchedules]
    B -->|No| F[Skip schedule provisioning]
```
...
sequenceDiagram
    ...
    participant Workflow as runScheduledSyncWorkflow
    participant Router as API Router
    participant Orch as sync-orchestrator-service

    Temporal->>Workflow: Schedule triggers
    Workflow->>Router: /cron/sync/orchestrate
    Router->>Orch: Orchestrate sync
    Orch->>Orch: Select candidates
    par Start workflows
        Orch->>Temporal: processSingleCity workflows
        Orch->>Temporal: processSingleCampus workflows
    end
```
...
sequenceDiagram
    participant Admin
    participant Router as /api/admin/sync/orchestrate
    participant Orch as sync-orchestrator-service
    participant Temporal

    Admin->>Router: POST orchestrate
    Router->>Orch: Run pipeline
    Orch->>Orch: Select candidates
    par Start workflows
        Orch->>Temporal: Start city/campus workflows
        Orch->>Temporal: Trigger overlay fold workflows
    end
```
...
sequenceDiagram
    ...
    Workflow->>Normalizer: Process OCR/ICS data
    Normalizer-->>Workflow: Normalized schedule
    Workflow->>DB: Store time_blocks
```
...
```mermaid
sequenceDiagram
    participant Workflow as scheduleRemoval.workflow
    participant DB as PostgreSQL

    Workflow->>DB: Mark dropped courses
    par Delete blocks
        Workflow->>DB: DELETE student_time_blocks
        Workflow->>DB: DELETE time_blocks
    end
```
...
- `services/engine/src/workflows/utils/calendarSyncLoadShedding.ts` (calendar shard + bucket + account dedupe keys)
- `services/engine/src/workflows/studentWatcher.simplified.workflow.ts` (calendar sync workflow id usage)
- `services/engine/src/activities/student.activities.ts` (account-level dedupe during Google sync)
...
- `services/api-router/src/routes/calendar-routes.ts` (manual calendar sync trigger + dedupe-aware fallback workflow IDs)
...
## Recent Changes
- 2026-02-10: Added sharded, window-bucketed calendar sync IDs and account-level dedupe keys to reduce duplicate fan-out under bursty trigger patterns.
