...
### iPhone Tab Navigation (Navigation IA Refactor - January 2026)

```mermaid
flowchart TB
    subgraph TabShellView
        direction TB
        T[TabView<br/>selection: selectedTab] --> T1["Today (.today)"]
        T --> T2["Tasks (.tasks)"]
        T --> T3["Schedule (.schedule)"]
        T --> T4["Courses (.courses)"]
        T --> T5["Library (.library)"]

        PB[Profile Button] --> SS[SettingsSheetView]
    end

    subgraph TabRegistry ["TabRegistry (Per-Tab Navigation)"]
        R[navigationPaths: Dictionary]
        R --> P1["today: NavigationPath"]
        R --> P2["tasks: NavigationPath"]
        R --> P3["schedule: NavigationPath"]
        R --> P4["courses: NavigationPath"]
        R --> P5["library: NavigationPath"]
    end
...
**iPhone Tab Structure (Unified Navigation IA):**

| Tab | AppTab | View | Purpose |
|-----|--------|------|---------|
| Today | `.today` | `TodayTabRootView` -> `HomeTabView` | Dashboard with unified widgets (UpNextCard, WidgetStackView) |
| Tasks | `.tasks` | `TasksTabRootView` -> `TasksTabView` | Task management |
| Schedule | `.schedule` | `ScheduleTabRootView` -> `ScheduleTabView` | Calendar, week/month views |
| Courses | `.courses` | `CoursesTabRootView` -> `TermTabView` | Course list with UniversalCourseCard |
| Library | `.library` | `LibraryTabRootView` -> `LibraryTabView` | Files/Notes with segmented picker |

**Settings Access:**
- Profile button (person.crop.circle icon) in top-right toolbar of each tab
- Opens `SettingsSheetView` as a sheet presentation
- Environment binding `showSettingsSheet` managed via `SettingsToolbarModifier`
...
**DORM-746/753 Fix (2026-01-04):** The `cityId` for channel subscriptions now comes from the backend `/v2/students/me` endpoint rather than hardcoded mappings. The backend derives `cityId` from the campus context hierarchy (campus → city via `parent_id`). iOS receives `cityId` in the `User` model and passes it to `RealtimeUpdatesService.startListening()`. This enables multi-campus support without iOS code changes when new campuses are added.
...
flowchart LR
    ...
    subgraph NotImplemented ["Not Implemented"]
        N1[Menu Bar Customization]
        N2[Multi-Window Support]
        N3[Touch Bar]
        N4[Right-Click Menus]
    end
```
...
| **Drag & Drop** | Fully Implemented | File upload via drop zone, security-scoped URL fallback | `IPadFilesView.swift`, `DocumentUploadService.swift` |
...
| **Keyboard Shortcuts** | Fully Implemented | Cmd+1-6 nav, Cmd+, settings, Cmd+K Ace | `IPadNavigationShell.swift:33-58` |
| **Sidebar Icons** | Fully Implemented | White icons when selected for visibility | `IPadSidebarView.swift` |
| **Sheet Cancel Buttons** | Fully Implemented | X icon instead of truncated "Cancel" text | `AddEventSheet.swift`, `EditEventSheet.swift`, etc. |
...
**Decision:** Functional Mac Catalyst support with keyboard-first productivity features.

**What's Implemented:**
- Title bar hiding (polish)
- Drag and drop file upload with security-scoped URL fallback (productivity)
- Keyboard shortcuts: Cmd+1-6 for navigation, Cmd+, for settings, Cmd+K for Ace (power users)
- Proper sidebar icon contrast when selected (visual polish)
- Sheet Cancel buttons use X icon to prevent text truncation (UI fix)
- Graceful feature degradation (stability)

**What's Not Implemented:**
- Menu bar customization (Go menu, File menu, etc.)
- Multi-window support
- Touch Bar integration
- Native right-click context menus

**Rationale:** The app is optimized for iOS/iPad touch-first experience. Mac Catalyst support provides a fully functional desktop option with keyboard shortcuts for power users, while deferring more complex Mac-native features like multi-window support.
...
**Implemented Shortcuts (as of 2026-01-04):**

| Shortcut | Action | File |
|----------|--------|------|
| Cmd+1 | Dashboard | `IPadNavigationShell.swift` |
| Cmd+2 | Term | `IPadNavigationShell.swift` |
| Cmd+3 | Schedule | `IPadNavigationShell.swift` |
| Cmd+4 | Tasks | `IPadNavigationShell.swift` |
| Cmd+5 | Library Files | `IPadNavigationShell.swift` |
| Cmd+6 | Library Notes | `IPadNavigationShell.swift` |
| Cmd+, | Settings | `IPadNavigationShell.swift` |
| Cmd+K | Ask Ace (AI) | `IPadNavigationShell.swift` |

**Implementation Pattern Used:**
```swift
// Hidden buttons with keyboard shortcuts in background
.background(
    Group {
        Button("") { selectedItem = .dashboard }
            .keyboardShortcut("1", modifiers: .command)
        Button("") { selectedItem = .term }
...
flowchart TB
    ...
    subgraph Results ["Search Results"]
        R1["📚 MATH 101 - Calculus"]
        R2["✅ Homework 3 - Due Tomorrow"]
        R3["📝 Lecture Notes: Derivatives"]
    end
...
flowchart TB
    subgraph NCWidgets ["Notification Center Widgets"]
        W1["📅 Next Class<br/>MATH 101 in 25 min<br/>Room 203"]
        W2["✅ Due Today (3)<br/>Essay Draft<br/>Problem Set 4<br/>Quiz Review"]
        W3["📊 Week Progress<br/>■■■■□□□ 57%"]
    end
```
...
| Date | Author | Changes |
|------|--------|---------|
| 2026-01-04 | Claude | **Navigation IA Refactor**: Unified 5-tab navigation (Today/Tasks/Schedule/Courses/Library), Settings as sheet via profile button |
| 2026-01-04 | Claude | **Unified Widget System**: CollapsibleWidgetContainer, UpNextCard, WidgetStackView for iPhone/iPad |
| 2026-01-04 | Claude | **UniversalCourseCard**: Dark glassmorphic styling with mesh gradients in DWDesign |
| 2026-01-04 | Claude | **LibraryTabView**: New Library tab with Files/Notes segmented picker |
| 2026-01-04 | Claude | DORM-746/753: cityId now from backend /me endpoint, removed hardcoded UMich mapping |
| 2026-01-04 | Claude | Implemented Mac Catalyst keyboard shortcuts (Cmd+1-6, Cmd+,, Cmd+K) |
| 2026-01-04 | Claude | Fixed sidebar icon visibility (white when selected for contrast) |
| 2026-01-04 | Claude | Fixed sheet Cancel buttons (X icon instead of truncated text on Catalyst) |
| 2026-01-04 | Claude | Fixed drag-drop upload (security-scoped URL fallback in DocumentUploadService) |
