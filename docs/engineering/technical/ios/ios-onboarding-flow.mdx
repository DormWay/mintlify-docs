...
sequenceDiagram
    ...
    App-->>User: "🌤️ Perfect Campus Weather!" + intelligence
    User->>App: Impressed → wants to authenticate
```
...
private func generateZeroStateHeadline(campus: Campus, weather: WeatherData) -> String {
    ...
    if weather.isPerfectWeather() {
        return "🌤️ Perfect Weather at \(campus.name)!"
    }
...
- iOS Architecture - PLG-first architectural overview
