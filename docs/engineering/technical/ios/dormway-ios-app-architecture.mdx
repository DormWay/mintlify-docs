...
- Uses SwiftUI's native navigation with `NavigationStack`
- Implements deep linking support via URL schemes
- Custom router pattern for complex navigation flows

```mermaid
graph LR
    A[TabShellView] --> B[Tab Bar]
    B --> C[Home Tab]
    B --> D[Schedule Tab]
    B --> E[Tasks Tab]
    B --> F[Term Tab]
...
**Current Tab Structure** (December 2025):
| Tab | View | Purpose |
|-----|------|---------|
| Home | `HomeTabView` | Dashboard with widgets |
| Schedule | `ScheduleTabView` | Day/week calendar view |
| Tasks | `TasksTabView` | Assignment tracking |
| Term | `TermTabView` | Course cards, term progress |
| Me | `MeTabView` | Profile, settings, integrations |
...
- **v4.0.0 (Current - December 2025)** - **iOS 26 PLATFORM FEATURES**: App Intents (Siri + Apple Intelligence), Live Activities (Dynamic Island class countdown), Control Center Widgets, AlarmKit (class alarms with per-course preferences)
