...
- **AI policy gating**: context prediction respects `preferences.ai.enabled` (and `ai.greenMode`). If AI is disabled or preferences are unavailable, the API returns a state-machine fallback and emits `context_prediction_completed` with `error_code`/`ai_policy_reason` of `ai_disabled` or `preferences_unavailable` (no LLM call).
