...
- Schedule rules resolve display timezone + placement constraints from user preferences, falling back to `accounts.metadata.timezone`.
- Web Settings store:
...
- Schedule UI reads `calendar.workHours`, `calendar.timeFormat`, and `calendar.weekStart`; show-weekend falls back to localStorage when no preference is provided.
...
- User intent may override quiet hours only if the request explicitly targets that range (tracked as `explicitWindow` on the rule metadata).
