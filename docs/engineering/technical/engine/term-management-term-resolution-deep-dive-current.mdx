...
description: "This document describes how DormWay **currently** models and resolves “the current term” across the system, including:"
---
...
- **Scheduling:** Triggered daily at 5:00 AM via the `sync-term-data-enrichment` Temporal schedule (see `services/engine/src/schedules/syncOrchestratorSchedules.ts`; gated by `SYNC_SCHEDULES_ENABLED`).
...
- `getForStudent(studentContextId, term, year)` matches enrollments by canonical term ID first (`${term.toLowerCase()}_${year}`), with legacy fallback (`term_id ILIKE %term%` + `term_year=year`).
- `getForStudent` and `getAllForStudent` include lifecycle dependency types:
  - `enrolled_in`, `completed`, `dropped`, `has_taken`
- Both methods rank dependencies per course context and keep one canonical row:
  - priority: `enrolled_in > completed > dropped > has_taken`
  - tie-breaker: latest `updated_at`
- Lifecycle output is normalized via `resolveCourseLifecycleState` in `@dormway/core`:
  - `completed` -> `status='completed'`, `isArchived=true`
  - `dropped` -> `status='dropped'`, `isArchived=true`
  - `has_taken` or explicit archived flags -> `status='archived'`, `isArchived=true`
- Missing `term_year` still reduces year-level separation for legacy rows.
...
flowchart LR
  ...
  subgraph Core["@dormway/core"]
    C1["TermFactory.getForStudent(term, year)\nfilters on context_dependencies.term_id/year + lifecycle deps"]
    C2["Term entity\nhas estimated dates\n(typical US patterns)"]
  end

  subgraph DB["Postgres"]
    D1["campus_configs\ncurrent_term_data + academic_calendar"]
    D2["context_dependencies\nterm_id + term_year + dependency_type + metadata.term"]
  end
...
1) **Multiple sources of “current term”**:
   ...
   - `@dormway/core` uses `context_dependencies.term_id` + `term_year` + lifecycle dependency states.
