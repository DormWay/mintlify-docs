...
- Read-model lifecycle contract: `.repos/dormway-platform/services/shared/dormway-core/src/entities/course-lifecycle.ts` (`resolveCourseLifecycleState`)
...
Read-model lifecycle contract (terms + APIv2 students/courses):

- `resolveCourseLifecycleState` (`@dormway/core`) is canonical for mapping dependency/archived inputs into `dependencyType`, `isArchived`, and `status`.
- `completed` and `dropped` are treated as archived lifecycle states on read (`isArchived=true`) with statuses `completed` and `dropped`.
- `has_taken` is treated as archived on read (`isArchived=true`, `status='archived'`).
- Term-facing reads in `TermFactory` include `enrolled_in`, `completed`, `dropped`, and `has_taken`, then select one canonical dependency row per course using priority ranking.
