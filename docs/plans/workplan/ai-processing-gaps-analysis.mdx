...
- ✅ Professor contact info (via `contact_extractor` agent)
- ✅ Office hours (via `contact_extraction_task`)
- ✅ Grading breakdown (via `grading_extractor` agent)
- ✅ Course policies (via `policy_analyst` agent)
- ✅ Professor style analysis (via `hidden_insights_agent`)
- ✅ Workload analysis (via `workload_analyst`)
- ✅ Cost analysis (via `cost_analyst`)
- ✅ Grade strategy (via `grade_strategist`)
