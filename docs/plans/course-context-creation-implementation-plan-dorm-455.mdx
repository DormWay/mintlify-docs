...
|--------|------------------------|--------------|----------------|
| **Schedule Import** | ✅ YES | Rich (course codes, sections) | `scheduleImport.workflow.ts:751-784` calls `createCourseContexts()` |
| **Canvas PAT** | ✅ YES | Rich (full LMS metadata) | `canvas.activities.ts:537` calls `createCourseContexts()` |
| **Syllabus Upload** | ✅ YES | Rich (extracted course info) | `syllabusProcessor.workflow.ts:1249,1753` calls `syllabusActivities.createCourseContext()` |
| **Google Calendar** | ❌ NO | Poor (just event titles) | `studentProcessor.workflow.ts:520-523` - `contextId: undefined` |
| **ICS Import** | ❌ NO | Poor (just event titles) | `scheduleImport.workflow.ts:57` → `normalizeAndStoreCalendarData(contextId: undefined)` |
...
const googleCalendarResult = await calendarActivities.normalizeAndStoreCalendarData(
  'google',
  calendarData.events,
  undefined, // No context ID for personal calendars  ← THIS IS THE GAP
  ...
);
```
...
- Schedule Import → `createCourseContexts()`
- Canvas PAT → `createCourseContexts()`
- Syllabus Upload → `syllabusActivities.createCourseContext()`

**Secondary Data (Poor Quality - Time Blocks Only):**
- Google Calendar → time blocks, NO contexts
- ICS Import → time blocks, NO contexts
- Canvas ICS Feed → time blocks, NO contexts (insufficient metadata)

**DORM-455 Implementation Must:**
1. Unify the three context creators into ONE `upsertCourseContext()` activity
2. Add fuzzy matching for calendar events → existing course contexts
...
- **Lines 326-548**: `createCourseEnrollments()` - manages student → course dependencies
...
### Pathway 1: Canvas → Schedule Import
...
sequenceDiagram
    ...
    Course Activity->>Course Activity: Normalize "ENTR 490 105 FA 2025" → "ENTR 490"
    ...
    Course Activity->>DB: INSERT enrollment (student → contextId1, type=enrolled_in)
...
### Pathway 2: Schedule → Canvas Sync
...
sequenceDiagram
    ...
    DB-->>Course Activity: contextId1
    Course Activity->>DB: INSERT enrollment (student → contextId1)
    ...
    Course Activity->>DB: UPSERT enrollment (student → contextId1)
```
...
### Pathway 3: Syllabus → Canvas Sync
...
### Pathway 4: Syllabus → Schedule Import
...
### Pathway 5: Google Calendar → Canvas
...
### Pathway 6: iPhone Calendar → Any Source
...
**Special Handling**:
- ICS files may contain recurring events → expand into instances
...
sequenceDiagram
    ...
    Course Activity->>DB: INSERT enrollment (studentA → contextIdA)
    ...
    Course Activity->>DB: INSERT enrollment (studentB → contextIdA)
...
|-------|----------------|--------|----------|------------|------------|-------|
| **courseCode** | 🥇 Highest | 🥈 Medium | 🥉 Low | 🚫 N/A | 🥇 Highest | User input always wins |
| **courseName** | 🥈 Medium | 🥉 Low | 🥇 Highest | 🚫 N/A | 🥈 Medium | Syllabus most accurate |
| **section** | 🥇 Highest | 🥈 Medium | 🥉 Low | 🚫 N/A | 🥇 Highest | User knows their section |
| **schedule** | 🥇 Highest | 🥉 Low | 🚫 N/A | 🥈 Medium | 🥇 Highest | User schedule > LMS |
| **location** | 🥇 Highest | 🥈 Medium | 🚫 N/A | 🥉 Low | 🥇 Highest | User schedule most current |
| **instructor** | 🥉 Low | 🥈 Medium | 🥇 Highest | 🚫 N/A | 🥉 Low | Syllabus has official name |
| **term** | 🥈 Medium | 🥉 Low | 🥇 Campus Config | 🚫 N/A | 🥈 Medium | Campus config is truth |
| **year** | 🥈 Medium | 🥉 Low | 🥇 Campus Config | 🚫 N/A | 🥈 Medium | Campus config is truth |
| **semesterDates** | 🥈 Medium | 🥉 Low | 🥇 Campus Config | 🚫 N/A | 🥈 Medium | Campus config is truth |
| **courseType** | 🥇 Auto-detect | 🥉 Low | 🥈 Medium | 🚫 N/A | 🥇 Auto-detect | Infer from schedule pattern |
| **canvas_course_id** | 🚫 N/A | 🥇 Only | 🚫 N/A | 🚫 N/A | 🚫 N/A | Canvas-only metadata |
| **canvas_enrollment_term_id** | 🚫 N/A | 🥇 Only | 🚫 N/A | 🚫 N/A | 🚫 N/A | Canvas-only metadata |

**Authority Legend**:
- 🥇 **Highest**: Wins in all conflicts
- 🥈 **Medium**: Wins if higher authority is null
- 🥉 **Low**: Only used if all higher authorities are null
- 🚫 **N/A**: Source doesn't provide this field
...
```typescript
// Input variations:
"ENTR 490"           → canonical: "ENTR-490", compact: "ENTR490"
"ENTR-490"           → canonical: "ENTR-490", compact: "ENTR490"
"ENTR490"            → canonical: "ENTR-490", compact: "ENTR490"
"ENTR 490 105"       → canonical: "ENTR-490", section: "105"
"ENTR-490-105-FA25"  → canonical: "ENTR-490", section: "105"
"CS101"              → canonical: "CS-101", compact: "CS101"
"CS 101"             → canonical: "CS-101", compact: "CS101"
...
```sql
-- ✅ CORRECT: Get student's courses (respects privacy)
SELECT c.*
FROM contexts c
...
-- ❌ WRONG: Query all courses at campus (exposes other students' data)
SELECT c.*
FROM contexts c
WHERE c.parent_id = :campusContextId
  AND c.type = 'course';

-- ✅ CORRECT: Get student's syllabus for a course
SELECT sd.*
FROM service_data sd
...
-- ❌ WRONG: Get all syllabi for a course (exposes other students' uploads)
SELECT sd.*
FROM service_data sd
...
### Phase 3: Syllabus → Course Linking (8 hours)
...
describe('Course Context Pathways', () => {
  it('Pathway 1: Canvas → Schedule', async () => {
    // Step 1: Canvas sync
...
- ✅ One course context per unique (campus, course code) combination
- ✅ Multiple sources (Canvas, schedule, syllabus, calendar) merge into single context
- ✅ Authority rules enforced (schedule > Canvas for section, etc.)
- ✅ Privacy maintained (student data separated from global course data)
- ✅ No duplicate courses created
- ✅ Orphaned syllabi and calendar events automatically linked

### Performance Requirements
- ✅ Course lookup < 100ms (with proper indexes)
- ✅ Deduplication check < 50ms per course
- ✅ Full course sync (50 courses) < 5 seconds
- ✅ No N+1 queries in API endpoints

### Data Quality Requirements
- ✅ 95%+ of courses have correct metadata (name, section, term)
- ✅ 90%+ of syllabi linked to course contexts
- ✅ 85%+ of calendar events linked to course contexts
- ✅ Zero duplicate course contexts for same (campus, code)
...
- [Engineering/Architecture/Database Schema](/docs/engineering/technical/database/context-system-database-schema) - Full schema reference
