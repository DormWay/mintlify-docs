...
## ✅ P0 - Must Ship (Instant-Start MVP)
...
## 🔄 P1 - High Impact Refinements (Ship Week 1)
...
- [ ] **Integrate sync service into notes page**
  ...
  - [ ] Show sync status in banner: "Synced • 0:12 ago"
...
- [ ] **Enhanced banner status**
  ```
  Auto-saving (local)  →  Syncing...  →  Synced • 0:12 ago
  ```
  - [ ] Hover tooltip: "Local drafts every second. Cloud sync every ~30s when online."
  - [ ] Distinct icons for each state
  - [ ] Color coding (yellow → blue → green)

### Class Detection & Context

- [ ] **Graceful detection with suggestions**
  - [ ] If no class detected → default to "Personal" ✓
  - [ ] If overlapping classes → show suggestion: "Looks like COMM 230?"
  - [ ] One-click to accept suggestion (updates context)
  - [ ] Never block typing - suggestions in banner only

- [ ] **Confidence scoring**
  - [ ] Low confidence (<70%) → show as suggestion, not auto-apply
  - [ ] High confidence (>90%) → auto-apply with "auto-detected" badge
  - [ ] Medium → default to most recent class or Personal
...
- [ ] **Keyboard shortcuts**
  ...
  - [ ] `⌘-Shift-1/2/3` → Quick headings (# ## ###)
  - [ ] `⌘-B` → Bold, `⌘-I` → Italic
  - [ ] `⌘-Shift-C` → Inline checklist toggle
  - [ ] `Escape` → Exit focus (blur editor)
...
- [ ] **Inbox → Notes handoff**
  ...
  - [ ] Template: "Prep for [topic]... My notes: —"
  - [ ] Pass `contextId` and `title` via URL params

- [ ] **Ace Chat → Notes handoff**
  - [ ] "💾 Save to current notes" button on Ace responses
...
- [ ] **Banner component**
  ...
  - [ ] Updates save status: saving → saved
  - [ ] Calls `onChangeClass` when "Change" clicked

### Integration Tests

- [ ] **E2E: Instant-start flow**
  - [ ] Open `/dashboard/notes` → cursor in editor immediately
...
- [ ] **E2E: Conflict resolution**
  ...
  - [ ] Close Tab A → sync
  - [ ] Close Tab B → sync (conflict detected)
...
- [ ] **Smoke test: Lecture scenario**
  ...
  3. ✅ Cursor in editor immediately (<300ms)
  4. ✅ Banner shows: "📚 DIGITAL 202 [auto-detected] • 45 min left"
  5. ✅ Start typing → "Auto-saving (local)" appears
  6. ✅ After 30s → "Syncing..." → "Synced • 0:12 ago"
  7. ✅ Close tab → reopen → draft restored

- [ ] **Smoke test: Outside class hours**
  1. Open `/dashboard/notes` at 11:00 PM
  2. ✅ Cursor in editor immediately
  3. ✅ Banner shows: "Personal" (no auto-detection)
  4. ✅ Can type immediately
  5. ✅ Click "Change" → modal opens → select class

- [ ] **Smoke test: Offline mode**
  1. Turn off network
  2. Type notes
  3. ✅ Local autosave works
  4. ✅ Banner shows: "Auto-saving (local)"
  5. Turn on network
  6. ✅ Banner shows: "Syncing..." → "Synced"
  7. ✅ Verify note in backend: `GET /api/notes/recent`
...
- [ ] **OCR for photos**: Not yet implemented
  - Future: Tap-to-add photo → OCR → insert text
...
### Backend
- ✅ `/api/mobile/calendar/current` - class detection
- ✅ `/api/notes/sync` - durable sync with conflict resolution
- ✅ `/api/notes/recent` - quick-resume notes
- ✅ Database migration for `notes` + `note_revisions` tables

### Frontend
- ✅ Auto-detection on page load
- ✅ Smart context banner (class, time, save status)
- ✅ Offline sync with retry queue
- ✅ Preserved existing autosave logic
...
[Attach: Banner showing "DIGITAL 202 [auto-detected] • 45 min left • Synced"]
...
**Status**: Ready for implementation → testing → rollout
